...
typedef struct _CH347_info { // 记录CH347引脚状态
    ...
    int buffer_idx;
    unsigned char buffer[HW_TDO_BUF_SIZE];
} _CH347_Info;
...
static int CH347_Write(void *oBuffer, unsigned long *ioLength)
{
    ...
    WI = 0;
    while (1) {
        ret = upWriteData(ugIndex, (uint8_t*)oBuffer + WI, &wlength);
        LOG_DEBUG_IO("(size=%lu, buf=[%s]) -> %lu", wlength, HexToString((uint8_t *)oBuffer, wlength), wlength);
        WI += wlength;
        if (WI >= *ioLength)
            break;
...
static int CH347_Read(void *oBuffer, unsigned long *ioLength)
{
    ...
    LOG_DEBUG_IO("(size=%lu, buf=[%s]) -> %lu", rlength, HexToString((uint8_t *)oBuffer, rlength), rlength);
    *ioLength = rlength;
    return true;
}
...
static int ch347_init(void)
{
    ...
    if (uhModule == 0) {
        ...
        if (uhModule) {
            upOpenDev = (pCH347OpenDevice)(void*)GetProcAddress(uhModule, "CH347OpenDevice");
            upCloseDev = (pCH347CloseDevice)(void*)GetProcAddress(uhModule, "CH347CloseDevice");
            upReadData = (pCH347ReadData)(void*)GetProcAddress(uhModule, "CH347ReadData");
            upWriteData = (pCH347WriteData)(void*)GetProcAddress(uhModule, "CH347WriteData");
            upSetTimeout = (pCH347SetTimeout)(void*)GetProcAddress(uhModule, "CH347SetTimeout");
            upJtagInit = (pCH347Jtag_INIT)(void*)GetProcAddress(uhModule, "CH347Jtag_INIT");
